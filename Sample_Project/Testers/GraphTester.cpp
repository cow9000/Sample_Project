...
void GraphTester::compareTraversals(){
    ...
    for(int i = 0; i < puzzle.size(); i++){
        int tempNum = puzzle.costTraversal(puzzle, i);
        if(tempNum < lowest){ lowest = tempNum; lowestIndex = i; }
        if(tempNum > highest){ highest = tempNum; highestIndex = i; }
    }
    
    std::cout << "Lowest cost - " << lowest << std::endl;
    std::cout << "Lowest index - " << lowestIndex << std::endl;
    std::cout << "Text - " << std::endl;
    puzzle.breadthFirstTraversal(puzzle, lowestIndex);
    std::cout << "Highest - " << highest << std::endl;
    std::cout << "Highest index - " << highestIndex<< std::endl;
    std::cout << "Text - " << std::endl;
    puzzle.breadthFirstTraversal(puzzle, highestIndex);
